<?php	
	...
	if(isset($_POST["level_start_choose"]) and isset($_POST["teacher_choose"]) and isset($_POST["timetable_choose"])
		...
		$sql = "SELECT `one lesson default` FROM `constants`" ;
			...
			// /ONE LESSON
			// print_r($arr_one_lesson);
			/*
		// LESSONS NUM
		for ($i=0; $i <count($arr_id); $i++) { 
			$sum_pay = 0;
			for ($f=0; $f < count($arr_pay[$i]); $f++) { 
				$sum_pay = $sum_pay + $arr_pay[$i][$f];
			}
			$discount = $arr_discounts[$i]*0.01;
			$one_lesson =$one_lesson_default - ($one_lesson_default*$discount);
			$lessons_num = floor($sum_pay/$one_lesson);
			$left = $sum_pay%$one_lesson;
			$arr_lessons_num[$i][0] = $sum_pay;
			$arr_lessons_num[$i][1] = $discount;
			$arr_lessons_num[$i][2] = $one_lesson;
			$arr_lessons_num[$i][3] = $lessons_num;
			$arr_lessons_num[$i][4] = $left;
			// echo "sum_pay : ".$sum_pay.PHP_EOL;
			// echo "discount : ".$discount.PHP_EOL;
			// echo "one_lesson : ".$one_lesson.PHP_EOL;
			// echo "lessons_num : ".$lessons_num.PHP_EOL;
		...
		for ($i=0; $i < count($arr_id); $i++) { 
			...
			// 	$arr_all[$arr_fio[$i]."|".$arr_id[$i]."|".$arr_person_start[$i]."|".$arr_person_stop[$i]]['freeze'] = $freeze_dates_arr[$i];
			// }

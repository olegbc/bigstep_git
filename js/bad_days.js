	...
	if(getParameterByName('teacher') && getParameterByName('timetable') && getParameterByName('level_start')){
		$('.brick input').each(function(){
			if($(this).val()==getParameterByName('teacher')){
				...
				findComboDays(teacher,timetable,level_start);
				$(this).parent('.brick').css('borderColor','blue');
			}
		});
	}
...
function findComboDays(teacher,timetable,level_start,shift){
	...
	$.ajax({
		...
		success: function(data) {
			...
			var yearMonthDay = [];
			for(i=0;i<21;i++){
				yearMonthDay[i] = data[0][i].split('-');
			}
			var badDayDates = [];
			for(i=0;i<data[1].length;i++){
				badDayDates[i] = data[1][i].split('-');
			}
			monthInput=0;
			monthOutput=0;
...
//	/построение шапки
	...
	$('.day').click(function(){
		...
		}else{alert('Если дата старта не подходящий день, перенесите дату старта в attendance_table');}
	});
	//	/бэд дэй в базу

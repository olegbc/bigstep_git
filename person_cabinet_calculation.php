<?php 
	...
	if(isset($_POST['person']) and $_POST['person']!=""){
		...
		$sql="SELECT `teacher`,`timetable`,`level_start`,`level` FROM `levels_person` WHERE id_person=".$_POST['person'];
		$result = mysql_query($sql)	or die(mysql_error());
		$sochitanie_arr = array(); // массив со всеми сочатаниями данной персоны
		$t=0;
		while($row = mysql_fetch_row($result)){
			$sochitanie_arr[$t][0] = $row[0];
			$sochitanie_arr[$t][1] = $row[1];
			$sochitanie_arr[$t][2] = $row[2];
			$sochitanie_arr[$t][3] = $row[3];
			$t++;
		}
		...
		if(count($sochitanie_arr)){
			...
			for($i=0;$i<count($sochitanie_arr);$i++){
				...
				// $sql="SELECT `level` FROM `levels_person` WHERE id_person=".$_POST['person']." AND teacher='".$sochitanie_arr[$i][0]."' AND timetable='".$sochitanie_arr[$i][1]."' AND level_start='".$sochitanie_arr[$i][2]."'";;
				// $result = mysql_query($sql)	or die(mysql_error());
				// $t=0;
				// while($row = mysql_fetch_row($result)){
				// 	$levels_arr[$t] = $row[0];
		...
		// BALANCE
	$sql="SELECT `balance` FROM `balance` WHERE id_person=".$_POST['person'];
	$result = mysql_query($sql) or die(mysql_error());
